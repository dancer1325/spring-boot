...
* allows
    ** about your application
        ...
        *** interacting
* available
    ** built-in by Spring Boot
        *** 👁️if they are available (-- check next bullet point --) -> are auto-configured 👁️
        *** Check xref:actuator/endpoints.adoc#actuator.endpoints.builtin[here]
    ** added by you
* allowed customizations
    ** xref:actuator/endpoints.adoc#actuator.endpoints.enabling[enable or disable] / individual endpoint
        *** if the endpoint is disabled -> is removed entirely from the application context
    ** xref:actuator/endpoints.adoc#actuator.endpoints.exposing[-- expose them ( == make them remotely accessible) over -- HTTP or JMX]
        *** if it's over HTTP -> endpoint -- is, by default, mapped to -- `/actuator/id`
            **** _Example:_ `/actuator/health`
        *** Check xref:api:rest/actuator/index.adoc[API documentation]
    ** available == enabled + exposed

[[actuator.endpoints.builtin]]
== Built-in Endpoints
...
| `auditevents`
| - Audit events information in current application

  - Requirements: `AuditEventRepository` bean.

| `beans`
| Complete list of all the Spring beans in your application.

| `caches`
| Available caches.

| `conditions`
| - Conditions / -- were evaluated on -- configuration and auto-configuration classes

  - reasons why they did or did not match

| `configprops`
| Collated list of all `@ConfigurationProperties` / subject to xref:actuator/endpoints.adoc#actuator.endpoints.sanitization[sanitization]

| `env`
| Properties from Spring's `ConfigurableEnvironment` / subject to xref:actuator/endpoints.adoc#actuator.endpoints.sanitization[sanitization].

| `flyway`
| - Flyway database migrations / have been applied.

- Requirements >=1 `Flyway` beans.

| `health`
| Application health information.

| `httpexchanges`
| - HTTP exchange information ( -- by default, last 100 HTTP request-response exchanges --)

  - Requirements: `HttpExchangeRepository` bean

| `info`
| Arbitrary application info.

| `integrationgraph`
| - Spring Integration graph

  - Requirements: dependency on `spring-integration-core`.

| `loggers`
| Configuration of loggers in the application.

| `liquibase`
| - Liquibase database migrations / have been applied.

  - Requirements >= 1 `Liquibase` beans.

| `metrics`
| "`metrics`" information for the current application.

| `mappings`
| Collated list of all `@RequestMapping` paths.

|`quartz`
| Information about Quartz Scheduler jobs / subject to xref:actuator/endpoints.adoc#actuator.endpoints.sanitization[sanitization].
...
| - Allows retrieval and deletion of user sessions -- from a -- Spring Session-backed session store.
  ...
  - Requirements: servlet-based web application / uses Spring Session.

| `shutdown`
| - gracefully shutdown the application -- by default, it's disabled

  - Requirements: jar packaging is used


| `startup`
| - xref:features/spring-application.adoc#features.spring-application.startup-tracking[startup steps data] collected by the `ApplicationStartup`

  - Requirements: `SpringApplication` -- must be configured with -- `BufferingApplicationStartup`.

| `threaddump`
| Performs a thread dump.
|===

* if your application is a web application (Spring MVC, Spring WebFlux, or Jersey), -> you can use also:
...
| - Returns a heap dump file.
  ...
  - if a HotSpot JVM -> `HPROF`-format file is returned

  - if OpenJ9 JVM -> `PHD`-format file is returned.

| `logfile`
| - Returns the contents of the logfile -- via -- HTTP `Range` header

- Requirements: `logging.file.name` or the `logging.file.path` property has been set


| `prometheus`
| - Exposes metrics in a format / can be scraped by a Prometheus server.

  - Requirements: dependency on `micrometer-registry-prometheus`.
|===
...
== Enabling Endpoints
* 👁️ By default, ALL endpoints except for `shutdown` are enabled. 👁️
* `management.endpoint.<id>.enabled: booleanToEnable`
...
* `management.endpoints.enabled-by-default`
    ** if `false` -> endpoint enablement to be opt-in & you need to enable individually / endpoint
        *** _Example:_
...
== Exposing Endpoints
* allows
    ** choosing the technologies over which an endpoint is exposed
* TODO:
...
* Check https://github.com/dancer1325/spring-guides-boot-actuator[thisTutorial]